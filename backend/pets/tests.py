...
class PetModelTestCase(TestCase):
    def setUp(self):
        ...
        self.pet_data = {"nome": "Rex", "especie": "Cachorro", "tutor": self.user}
...
class PetSerializerTestCase(TestCase):
    ...
    def test_valid_serializer(self):
        ...
        from .serializers import (
            PetSerializer,
        )  # Importação aqui para evitar circular references se serializer precisar do models

        serializer = PetSerializer(data=self.valid_data)
        self.assertTrue(
            serializer.is_valid(), serializer.errors
        )  # Passa erros para depuração
    ...
    def test_read_only_fields(self):
        ...
        data_with_readonly[
            "created_at"
        ] = "2020-01-01T00:00:00Z"  # Tentando definir created_at

        serializer = PetSerializer(data=data_with_readonly)
        self.assertTrue(
            serializer.is_valid(), serializer.errors
        )  # Deve ser válido, campos read-only são ignorados

        pet = serializer.save()
        self.assertNotEqual(
            pet.id, 9999
        )  # O ID deve ser gerado pelo banco, não o que passamos
    ...
    def test_image_upload(self):
        ...
        self.client.force_authenticate(
            user=self.user
        )  # Autentica o cliente para a requisição

        response = self.client.post(
            "/api/pets/", data=data, format="multipart"  # Crucial para upload de arquivos
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.json())
        self.assertIn(
            "foto", response.json()
        )  # Verifica se a URL da foto está na resposta
